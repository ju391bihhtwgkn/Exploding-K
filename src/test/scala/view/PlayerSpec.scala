...
class PlayerSpec() extends AnyWordSpec with Matchers {
  ...
  "A Player" when {
    "set to Julian" should {
      val player = Player("Julian", List(Card("Melon Cat", "Combine 2 or 3 to draw a card from a Player")))
      "have a name" in {
  ...
  "A Player" when {
    "at the start of the game" should {
      val player = Player("Julian", List(Card("Cat Card", "Combine 2 or 3 to draw a card from a Player")))
      "have not lost" in {
